...
export default function GeneratePage() {
  ...
  useEffect(() => {
    promptRef.current = sessionStorage.getItem(`prompt:${sessionId}`) ?? ''
  }, [sessionId])
  ...
  useEffect(() => {
    ...
    const es = new EventSource(`${API_BASE}/api/generate/${sessionId}`)
    ...
    es.onmessage = (e) => {
      ...
      try {
        ...
        if (raw.type === 'COMPLETE') {
          ...
          if (raw.type === 'PLANNING') {
            setProgress(10)
          }
          if (raw.type === 'CHAPTER') {
            chapterCount.current += 1
            // Estimate progress: planning=10%, each chapter adds ~22%
            setProgress(Math.min(90, 10 + chapterCount.current * 20))
          }
          if (raw.type === 'AUDIO') {
            // Whole-story audio — store URL separately, not as an inline block
            if (raw.url) setAudioUrl(raw.url as string)
          } else if (raw.type === 'GENERATED_IMAGE') {
            // Replace the most recent IMAGE skeleton with the real image
            setBlocks((prev) => {
        ...
        <div className="max-w-2xl mx-auto px-4 pb-20">
          <div className="rounded-2xl p-8 text-center border"
            ...
            <div className="flex flex-wrap gap-3 justify-center">
              ...
              {/* Copy share link (only once saved) */}
              {saveState === 'saved' && shareUrl && (
                <button
                  onClick={handleCopy}
      ...
      >
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-[#7c746d] text-xs mt-0.5">Scroll down to share or read aloud.</p>
          </div>
          <button
